...
Finish = Text(text = "LEVEL 2 COMPLETED!", scale = 5, color = color.white66, x=0, y=0,font='VeraMono.ttf')
...
def update():
    ...
    for i in range(0,20): #pushing the player on colliding with the obstacles
        if voxel2[i].x >= 119 or voxel2[i].x<=-4:
            v2p[i] *= -1
        if player.intersects(voxel2[i]).hit:
            if v2p[i] == -1:
                ...
                player.z = voxel2[i].z-2.2
        voxel2[i].position = (voxel2[i].x + 0.2*v2p[i], 6, voxel2[i].z)


    if hit_info.hit: #color changing and destruction of the platforms
        r = rd.randint(0,1)
        if ent != hit_info.entity:
            if hit_info.entity.color == color.orange: #CHANGE
        ...
        if hit_info.entity==ent2 and flag1==0:
            ...
            if j>3:
                ...
                if mycon.is_connected==False:
                    ...
                    myQry="UPDATE PlayerData set Level = 3, HighScore2 = "+Final_time+" where User_ID = %s and Password = %s and ("+Final_time+"<HighScore2)"
